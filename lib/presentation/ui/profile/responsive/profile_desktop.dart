...
class ProfilePageDesktop extends StatefulWidget {
  ...
  const ProfilePageDesktop({super.key, this.payload});
...
class _ProfilePageDesktopState extends State<ProfilePageDesktop> {
    ...
    return MultiBlocProvider(
      ...
      child: BlocBuilder<AuthBloc, AuthState>(
        builder: (context, state) {
          if (state.status != AuthStatus.authenticated&&widget.payload==null) {
            return Scaffold(
                appBar: AppBar(
          ...
          return StreamBuilder<User>(
              initialData: widget.payload?.user,
              stream: userRepository.userDataStream(
                  widget.payload?.user.id ?? authRepository.currentUser!.uid),
                      ...
                      builder: (context, state) {
                        if ((state.status == AuthStatus.authenticated &&
                            ...
                            (state.status == AuthStatus.notAuthenticated &&
                                widget.payload != null)) {
                          return Text(
                            '${userData?.name ?? LocaleKeys.title_profile.tr()} desktop',
                            style: Theme.of(context).textTheme.titleLarge,
                          );
                        }
                        return Text(
                  ...
                  SliverToBoxAdapter(
                    child: Column(
                        ...
                        children: [
                          ...
                          bioSectionView(
                              uid: widget.payload?.user.id ??
                                  authState.user!.id),
                          gameFavView(
                              widget.payload?.user.id ?? authState.user!.id),
                          SizedBox(
                            height: Dimens.DIMENS_8,
                          ),
                          if ((widget.payload?.user.id ?? authState.user!.id) ==
                              authRepository.currentUser?.uid)
                            Row(
                            ...
                            Row(
                              children: [
                                ...
                                widget.payload == null
                                    ? Container()
                                    : FutureBuilder<bool>(
                                        future: userRepository.isFollowing(
                                            widget.payload!.user.id),
                                        builder: (context,
  ...
  bool _isShowMenuBtn(AuthRepository authRepository, AuthState authState) =>
      authRepository.currentUser?.uid == widget.payload?.user.id ||
      widget.payload == null;
      ...
      children: [
              ...
              BlocBuilder<AuthBloc, AuthState>(
                builder: (context, state) {
                              ...
                              builder: (_) {
                                return Dialog(
                                  ...
                                  child: CachedNetworkImage(
                                    width: 300,
                                    imageUrl: widget.payload!.user.photo!,
                  ...
                  return StreamBuilder<String>(
                      stream:
                          repository.getAvatar(widget.payload?.user.id ?? uid!),
                      builder: (context, snapshot) {
        ...
        BlocBuilder<AuthBloc, AuthState>(
          builder: (_, state) {
            return FutureBuilder<int>(
                initialData: 0,
                future: repository.getFollowerCount(
                    widget.payload?.user.id ?? state.user!.id),
                      ...
                      onTap: () {
                        context.push(APP_PAGE.followingNFonllowers.toPath,
                            extra: FollowingNFollowersData(
                                initialIndex: 0,
                                userName: widget.payload?.user.userName ??
                                    state.user!.userName!,
                                uid:
                                    widget.payload?.user.id ?? state.user!.id));
                      },
        ...
        BlocBuilder<AuthBloc, AuthState>(
          builder: (context, state) {
            return FutureBuilder<int>(
                initialData: 0,
                future: repository.getFollowingCount(
                    widget.payload?.user.id ?? state.user!.id),
        ...
        BlocBuilder<AuthBloc, AuthState>(
          builder: (context, state) {
            return FutureBuilder<int>(
                initialData: 0,
                future: repository
                    .getLikesCount(widget.payload?.user.id ?? state.user!.id),
                builder: (context, AsyncSnapshot<int> snapshot) {
