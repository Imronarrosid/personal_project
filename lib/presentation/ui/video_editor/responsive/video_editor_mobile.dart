...
// class VideoEditorMobile extends StatefulWidget {
//   const VideoEditorMobile({super.key, required this.file});

//   final XFile? file;

//   @override
//   State<VideoEditorMobile> createState() => _VideoEditorMobileState();
// }

// class _VideoEditorMobileState extends State<VideoEditorMobile> {
...
//   void initState() {
//     super.initState();
//     videoFile = uploadRepository.pickedVideo;
//     if (videoFile != null) {
...
//     _controller.dispose();
//     try {
//       int fileSize = _fileMBSize(await (uploadRepository.exportedVideo ?? _controller.file).readAsBytes());
//       bool isMoreThan12MB = 12 < fileSize;
//       bool under30secButMoreThan5MB = (_controller.video.value.duration.inSeconds < 30 && fileSize > 5);
...
//         video = await exportVideo(
//           customInstruction: " -crf 28 -c:v libx264 -c:a aac -b:v 1250k -b:a 192k ",
//           onStatistics: (stats) =>
//               _exportingProgress.value = stats.getProgress(_controller.trimmedDuration.inMilliseconds),
//         );
//         debugPrint('video size ${_fileMBSize(await video.readAsBytes())}');
//         uploadRepository.storeExportedVideo(video);
//         if (mounted) {
//           context.go(
...
//         context.go(
//           APP_PAGE.upload.toPath + APP_PAGE.addDetails.toPath,
//           extra: AddDetails(videoFile: File(uploadRepository.exportedVideo!.path), thumbnail: file),
//         );
//       }
...
//         AppRouter appRouter = Provider.of(context, listen: false);
//         _removeFile();
//         _controller.video.pause();
//         appRouter.onBackButtonPressed(context);
//         uploadRepository.removeExportedFile();
//         return true;
//       },
//       child: Scaffold(
//         extendBody: true,
...
//                                               tooltip: LocaleKeys.label_rotate_counterclockwise.tr(),
//                                             ),
//                                           ),
//                                           Expanded(
//                                             child: IconButton(
...
//                         color: Theme.of(context).colorScheme.primary, borderRadius: BorderRadius.circular(50)),
//                     child: Text(
...
//   void _removeFile() {
//     File(uploadRepository.pickedVideo!.path).deleteSync(recursive: true);
//   }
...
//     final outputPath = kIsWeb ? webOutputPath('mp4') : await ioOutputPath(inputPath, outputFormat);
...
//       outputPath: kIsWeb ? quotedOutputPath.replaceAll('"', '') : quotedOutputPath,
...
//     final outputPath = kIsWeb ? webOutputPath('jpg') : await ioOutputPath(coverFile.path, outputFormat);
